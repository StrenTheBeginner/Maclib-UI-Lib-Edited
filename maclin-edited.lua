...
local MacLib = { 
	...
	GetService = function(service)
		local ok, inst = pcall(game.GetService, game, service)
		if ok and inst then
			return safeClone(inst)
		end
		...
		function SectionFunctions:Tab(Settings)
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					local orderBadgeColor
					local formatOrderHeaderText
					local refreshOrderBadges

					local function normalizeOptionList(raw)
						table.clear(OrderedOptions)
						table.clear(OptionLookup)
						...
						if #raw > 0 then
							for i, item in ipairs(raw) do
								...
								if typeof(item) == "table" then
									value = item.Value
									if value == nil then value = item.Id end
									if value == nil then value = item.Key end
									label = item.Text or item.Name or item.Label or item.Title
									if label == nil then
										label = tostring(value ~= nil and value or i)
									end
								...
								local entry = { Index = i, Label = label, Value = value }
								OrderedOptions[i] = entry
								OptionLookup[label] = entry
								local s = tostring(value)
								OptionLookup[s] = entry
								if typeof(value) == "number" then
									OptionLookup[value] = entry
								end
							end
						else
							local sortedKeys = {}
							...
							for idx, k in ipairs(sortedKeys) do
								local label = tostring(raw[k])
								local entry = { Index = idx, Label = label, Value = k }
								OrderedOptions[idx] = entry
								OptionLookup[label] = entry
								OptionLookup[tostring(k)] = entry
								if typeof(k) == "number" then
									OptionLookup[k] = entry
								end
							end
						end
					end

					local function resolveOptionChoice(choice)
						if choice == nil then
							return nil
						end
						if typeof(choice) == "number" then
							if OrderedOptions[choice] then
								return OrderedOptions[choice]
							end
							for _, entry in ipairs(OrderedOptions) do
								if entry.Index == choice or entry.Value == choice then
									return entry
								end
							end
							return nil
						end
						return OptionLookup[tostring(choice)]
					end

					local function getEntryByLabel(label)
						local obj = OptionObjs[label]
						return obj and obj.Entry
					end

					local function fireCallback()
						if not DropdownFunctions.Settings.Callback then
							return
						end
						if DropdownFunctions.Settings.Multi then
							...
							local value = entry and entry.Value or label
							local index = entry and entry.Index
							if DropdownFunctions.Settings.ReturnBoth then
								DropdownFunctions.Settings.Callback(value, index, label)
							elseif DropdownFunctions.Settings.ReturnIndex then
								DropdownFunctions.Settings.Callback(index, value, label)
							else
								DropdownFunctions.Settings.Callback(value, index)
							end
						end
					end
					...
					orderBadgeColor = function(rank)
						return ORDER_BADGE_COLORS[((rank - 1) % #ORDER_BADGE_COLORS) + 1]
					end

					formatOrderHeaderText = function()
					...
					local function Toggle(optionName, State)
						...
						if useOrder and refreshOrderBadges then
							refreshOrderBadges()
						elseif dropdownName then
							if #Selected > 0 then
					...
					local function addOption(entry)
						local i = entry.Index
						local v = entry.Label
						...
						if DropdownFunctions.Settings.Default ~= nil then
							if DropdownFunctions.Settings.Multi then
								local list = typeof(DropdownFunctions.Settings.Default) == "table"
									and DropdownFunctions.Settings.Default or { DropdownFunctions.Settings.Default }
								for _, choice in ipairs(list) do
									local defEntry = resolveOptionChoice(choice)
									if defEntry and defEntry.Label == v then
										isSelected = true
										break
									end
									if choice == v or choice == entry.Value or choice == i then
										isSelected = true
										break
									end
								end
							else
								local defEntry = resolveOptionChoice(DropdownFunctions.Settings.Default)
								isSelected = defEntry and defEntry.Label == v
							end
						end
						Toggle(v, isSelected)
					...
					if DropdownFunctions.Settings.Options then
						normalizeOptionList(DropdownFunctions.Settings.Options)
						for _, entry in ipairs(OrderedOptions) do
							addOption(entry)
						end
					end
					...
					function DropdownFunctions:UpdateSelection(newSelection)
						...
						if type(newSelection) == "table" and not DropdownFunctions.Settings.Multi then
							newSelection = newSelection[1]
						end
						if type(newSelection) == "table" then
							for _, choice in ipairs(newSelection) do
								local entry = resolveOptionChoice(choice)
								if entry then
									Toggle(entry.Label, true)
									table.insert(selectedOptions, entry.Label)
								end
							end
						else
							local entry = resolveOptionChoice(newSelection)
							if entry then
								Toggle(entry.Label, true)
								table.insert(selectedOptions, entry.Label)
							else
					...
					function DropdownFunctions:InsertOptions(newOptions)
						...
						normalizeOptionList(newOptions)
						for _, entry in ipairs(OrderedOptions) do
							addOption(entry)
						end
					end
					...
					function DropdownFunctions:GetValue()
						if DropdownFunctions.Settings.Multi then
							...
							for _, label in ipairs(Selected) do
								local entry = getEntryByLabel(label)
								table.insert(values, entry and entry.Value or label)
							end
							return values
						end
						local label = Selected[1]
						local entry = label and getEntryByLabel(label)
						return entry and entry.Value or label
					end
					function DropdownFunctions:GetIndex()
						if DropdownFunctions.Settings.Multi then
							local indexes = {}
							for _, label in ipairs(Selected) do
								local entry = getEntryByLabel(label)
								table.insert(indexes, entry and entry.Index)
							end
							return indexes
						end
						local label = Selected[1]
						local entry = label and getEntryByLabel(label)
						return entry and entry.Index
					end
					function DropdownFunctions:GetLabel()
						if DropdownFunctions.Settings.Multi then
							return Selected
						end
						return Selected[1]
				...
				function SectionFunctions:Select(Settings, Flag)
					return self:Dropdown(Settings, Flag)
				end

				function SectionFunctions:ComboBox(Settings, Flag)
				...
				function SectionFunctions:Chooser(Settings, Flag)
					Settings = Settings or {}
					if Settings.AsDropdown == true or Settings.UseDropdown == true then
						return self:Dropdown(Settings, Flag)
					end

					local ChooserFunctions = { Settings = Settings, IgnoreConfig = false, Class = "Chooser", Value = nil }
					local optionButtons = {}
					local selectedLabel = nil

					local chooser = Instance.new("Frame")
					chooser.Name = "Chooser"
					chooser.AutomaticSize = Enum.AutomaticSize.Y
					chooser.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					chooser.BackgroundTransparency = 0.985
					chooser.BorderSizePixel = 0
					chooser.Size = UDim2.new(1, 0, 0, 0)
					chooser.Parent = section

					local chooserPadding = Instance.new("UIPadding")
					chooserPadding.PaddingLeft = UDim.new(0, 15)
					chooserPadding.PaddingRight = UDim.new(0, 15)
					chooserPadding.PaddingTop = UDim.new(0, 8)
					chooserPadding.PaddingBottom = UDim.new(0, 8)
					chooserPadding.Parent = chooser

					local chooserLayout = Instance.new("UIListLayout")
					chooserLayout.Padding = UDim.new(0, 6)
					chooserLayout.SortOrder = Enum.SortOrder.LayoutOrder
					chooserLayout.Parent = chooser

					local chooserTitle = Instance.new("TextLabel")
					chooserTitle.BackgroundTransparency = 1
					chooserTitle.FontFace = Font.new(assets.interFont)
					chooserTitle.Text = Settings.Name or "Choose"
					chooserTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
					chooserTitle.TextSize = 13
					chooserTitle.TextTransparency = 0.35
					chooserTitle.TextXAlignment = Enum.TextXAlignment.Left
					chooserTitle.AutomaticSize = Enum.AutomaticSize.Y
					chooserTitle.Size = UDim2.new(1, 0, 0, 18)
					chooserTitle.Parent = chooser

					local ordered = {}
					local lookup = {}
					local raw = Settings.Options or {}
					if #raw > 0 then
						for i, item in ipairs(raw) do
							local label, value
							if typeof(item) == "table" then
								value = item.Value or item.Id or item.Key
								label = item.Text or item.Name or item.Label or tostring(value or i)
								if value == nil then value = label end
							else
								label = tostring(item)
								value = item
							...
							ordered[i] = entry
							lookup[label] = entry
							lookup[tostring(value)] = entry
							if typeof(value) == "number" then lookup[value] = entry end
						end
					else
						local keys = {}
						for k in pairs(raw) do table.insert(keys, k) end
					...
					local function resolveChoice(choice)
						...
						if typeof(choice) == "number" then
							return ordered[choice] or lookup[choice]
						end
						return lookup[tostring(choice)]
					end

					local function paintSelection()
						for label, btn in pairs(optionButtons) do
							local active = label == selectedLabel
							btn.BackgroundTransparency = active and 0.88 or 0.95
							btn.TextTransparency = active and 0.05 or 0.45
						end
					end

					local function chooseEntry(entry, fireCb)
						if not entry then return end
						selectedLabel = entry.Label
						ChooserFunctions.Value = entry.Value
						paintSelection()
						if fireCb and Settings.Callback then
							if Settings.ReturnBoth then
								Settings.Callback(entry.Value, entry.Index, entry.Label)
							elseif Settings.ReturnIndex then
								Settings.Callback(entry.Index, entry.Value, entry.Label)
							else
								Settings.Callback(entry.Value, entry.Index)
							end
						end
					end
					...
					for _, entry in ipairs(ordered) do
						...
						btn.Name = "Choice_" .. tostring(entry.Index)
					...
					if Settings.Default ~= nil then
						chooseEntry(resolveChoice(Settings.Default), false)
					end

					function ChooserFunctions:UpdateSelection(newSelection)
						chooseEntry(resolveChoice(newSelection), true)
					end
					function ChooserFunctions:SetOptions(newOptions)
						self:ClearOptions()
						self:InsertOptions(newOptions)
					end
					function ChooserFunctions:UpdateOptions(newOptions)
						self:SetOptions(newOptions)
					end
					function ChooserFunctions:InsertOptions(newOptions)
						...
						for i, item in ipairs(raw) do
							...
							local value = item
							if typeof(item) == "table" then
								value = item.Value or item.Id or item.Key or label
								label = item.Text or item.Name or item.Label or tostring(value)
							end
							local entry = { Index = i, Label = label, Value = value }
							ordered[i] = entry
							lookup[label] = entry
					...
					function ChooserFunctions:ClearOptions()
						for _, btn in pairs(optionButtons) do btn:Destroy() end
						table.clear(optionButtons)
						selectedLabel = nil
						ChooserFunctions.Value = nil
					end
					function ChooserFunctions:GetValue()
						return ChooserFunctions.Value
					end
					function ChooserFunctions:SetVisibility(state)
						chooser.Visible = state
					end

					if Flag then
						MacLib.Options[Flag] = ChooserFunctions
					end
					return ChooserFunctions
				end
